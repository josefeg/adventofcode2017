...
def checksum(spreadsheet: str) -> int:
    sum = 0
    lines = spreadsheet.splitlines()
    for line in lines:
        digits = list(map(lambda x: int(x), re.split(r"\s+", line)))
        sum += int(max(digits)) - int(min(digits))

    return sum


def find_evenly_divisible(digits: list) -> (int, int):
    for divisor in range(len(digits)):
        for dividend in range((len(digits) - 1), (divisor + 1), -1):
            if digits[dividend] % digits[divisor] == 0:
                return digits[dividend], digits[divisor]


def checksum2(spreadsheet: str) -> int:
    sum = 0
    lines = spreadsheet.splitlines()
    for line in lines:
        digits = sorted((map(lambda x: int(x), re.split(r"\s+", line))))
        dividend, divisor = find_evenly_divisible(digits)
        sum += int(dividend / divisor)
...
127	375	330	257	220	295	145	335	304	165	151	141	289	256	195	272"""
    ...
    print("checksum2 =>", checksum2(input))
