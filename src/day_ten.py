...
# a mental note to remind the Elves later not to invent their own
...
# otation. Because each number in your dense hash will be between 0 and 255
...
def calculate_hash_round(lengths: list, numbers: list, starting_position=0, initial_skip_size=0) -> int:
    ...
    for length in lengths:
        if (current_position + length) < len(numbers):
            ...
            sublist = numbers[current_position:] + numbers[0:((current_position + length) - len(numbers))]
        sublist.reverse()

        for i in range(length):
            index = current_position + i
            while index >= len(numbers):
                index = index - len(numbers)
            numbers[index] = sublist[i]
...
def calculate_sparse_hash(lengths: list) -> list:
    position = 0
    skip_size = 0
    numbers = list(range(256))
    for i in range(64):
        numbers, position, skip_size = calculate_hash_round(lengths,
                                                            numbers,
                                                            starting_position=position,
                                                            initial_skip_size=skip_size)
    return numbers


def calculate_dense_hash(sparse_hash: list) -> list:
    dense_hash = []
...
def calculate_hash(input_string):
    lengths = list(map(lambda x: ord(x), input_string))
...
def main():
    ...
    numbers, _, _ = calculate_hash_round(map(lambda x: int(x), input.split(",")), list(range(256)))
    value = numbers[0] * numbers[1]
    print(value)

    print(calculate_hash(input))
