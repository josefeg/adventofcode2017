...
def find_steps_for_exit(instructions: list, decrease_if_three_or_more=False) -> int:
    steps = 0
    ip = 0
    while True:
        steps += 1
        current_instruction = ip
        ip = ip + instructions[current_instruction]
        if instructions[current_instruction] >= 3 and decrease_if_three_or_more:
            instructions[current_instruction] = instructions[current_instruction] - 1
        else:
...
def main():
    ...
    with open(input_file) as f:
        instructions = list(map(lambda x: int(x), f.readlines()))

    print("steps_to_exit    => ", find_steps_for_exit(instructions[:]))
    print("steps_to_exit v2 => ", find_steps_for_exit(instructions[:], True))
