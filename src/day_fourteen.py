...
try:
    ...
    import day_ten as day_ten


def build_grid(key_string: str) -> list:
    grid = []
    for i in range(128):
        hash_input = "{}-{}".format(key_string, i)
        hash_value = int(day_ten.calculate_hash(hash_input), 16)
        grid.append(format(hash_value, "0128b"))
    return grid


def count_used_squares(key_string: str) -> int:
    used_squares = 0
    grid = build_grid(key_string)
    for line in grid:
        used_squares += line.count("1")
    return used_squares


def count_regions(key_string: str) -> int:
    regions = 0
    grid = build_grid(key_string)
    for y in range(128):
        line = grid[y]
...
def main():
    # key_string = "jzgqcdpd"
    key_string = "flqrgnkx"
    # print("used_squares =>", count_used_squares(key_string))
    print("regions      =>", count_regions(key_string))
