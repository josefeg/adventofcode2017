...
def has_no_anagrams(phrase: str) -> bool:
    words = list(map(lambda word: ''.join(sorted(word)), re.split("\s+", phrase)))
    words_with_no_anagrams = set(words)
    return len(words) == len(words_with_no_anagrams)
