...
def next_digit(index: int, sequence: str, distance: int = 1) -> str:
    next_index = index + distance
    if next_index >= len(sequence):
        next_index = next_index - len(sequence)
    return sequence[next_index]


def captcha(sequence: str) -> int:
    sum = 0
    for i in range(len(sequence)):
        if sequence[i] == next_digit(i, sequence):
            sum += int(sequence[i])
...
def captcha2(sequence: str) -> int:
    sum = 0
    distance = int(len(sequence) / 2)
    for i in range(len(sequence)):
        if sequence[i] == next_digit(i, sequence, distance):
            sum += int(sequence[i])
...
def main():
    ...
    print("captcha  =>", captcha(input))
    print("captcha2 =>", captcha2(input))
