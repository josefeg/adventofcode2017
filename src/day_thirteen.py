...
def find_scanner_position(firewall: map, layer: int, time: int) -> int:
    if time == 0:
        return 0

    depth = firewall[layer]
    if time >= depth:
        passes = ((time - depth) // (depth - 1)) + 1
        offset = time - ((passes - 1) * (depth - 1) + depth)
    else:
        passes = 0
        offset = time

    if (passes % 2) == 0:
        position = offset if time <= depth else offset + 1
    else:
        position = depth - (2 + offset)

    return position


def get_severity(lines: list, delay=0, stop_on_caught=False) -> int:
    severity = 0
    firewall = {int(x[0]): int(x[1]) for x in map(lambda x: x.split(": "), lines)}
    for time in range(delay, max(firewall.keys()) + (delay + 1)):
        layer = time - delay
        if layer not in firewall:
            continue

        scanner_position = find_scanner_position(firewall, layer, time)
        if scanner_position == 0:
            current_severity = time * firewall[layer]
            if stop_on_caught:
...
def find_smallest_delay(lines: list) -> int:
    delay = 1
    while True:
        severity = get_severity(lines, delay=delay, stop_on_caught=True)
        if severity == 0:
            return delay
        delay += 1
...
def main():
    ...
    print("severity       =>", get_severity(lines))
    print("smallest delay =>", find_smallest_delay(lines))
